-- ============================================================================
-- 0017_my_home_attempts.sql — เพิ่มจำนวนครั้งที่ทำ + คะแนนดีสุด ในงานที่ได้รับมอบหมาย
-- ============================================================================
...
begin
  ...
  return json_build_object(
    ...
    'assignments', (select coalesce(json_agg(json_build_object(
        'type', a.item_type, 'due', a.due, 'item_id', a.item_id,
        ...
        'attempts', case when a.item_type = 'quiz'
                         then (select count(*) from app.attempts aa where aa.user_id = v.id and aa.quiz_id = a.item_id) else 0 end,
        'best', case when a.item_type = 'quiz'
                     then (select max(aa.pct) from app.attempts aa where aa.user_id = v.id and aa.quiz_id = a.item_id) else null end,
